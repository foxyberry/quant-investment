...
1. `코드 -> PR -> 재수정`을 LGTM이 나올 때까지 반복하되, 중간 과정은 커밋하지 않는다.
- 마지막에 한 번만 PR 코멘트로 전체 수정 내용을 요약한다.

2. 짧은 시간에 많은 요청이 발생할 수 있는 지시(예: `이슈 100개 등록`)가 와도 그대로 100개를 바로 생성하지 않는다.
- 그룹(배치)으로 묶어서 처리한다.

3. 각 이슈 생성 사이에는 최소 30초 이상의 간격을 둔다.

4. 이슈 처리 중 각 PR 머지 작업 사이에도 최소 30초 이상의 간격을 둔다.

## 0-2. 이슈 작업 표준 워크플로우 (Mandatory for Claude)

이슈를 할당받거나 작업 지시를 받으면, 사용자의 추가 지시 없이 아래 6단계를 **자동으로** 순차 실행한다.

### 워크플로우

```
1. 방향성 리뷰 → 2. 구현 위임 → 3. 코드 리뷰 → 4. 지적사항 수정 → 5. 머지 → 6. 다음 이슈
```
...
| 단계 | 수행 주체 | 행동 | 완료 기준 |
|------|-----------|------|-----------|
| **1. 방향성 리뷰** | Claude + Codex | 이슈 읽기 → 관련 코드 탐색 → Codex MCP에 구현 방향 리뷰 요청 | Codex가 "방향 LGTM" 또는 피드백 반영 완료 |
| **2. 구현 위임** | Codex MCP | 브랜치 생성 → Codex에 구현 위임 (목표/파일/제약 전달, 코드는 Codex가 작성) | ESLint clean + build pass + 테스트 통과 |
| **3. 코드 리뷰** | Codex MCP | 변경사항 diff를 Codex에 리뷰 요청 | Codex가 "LGTM" 응답 |
| **4. 지적사항 수정** | Claude | Codex 리뷰에서 지적된 사항 수정 → 재검증 | High/Medium 이슈 모두 해결 (Low는 판단하여 수용/무시) |
| **5. 머지** | Claude | 커밋 → push → PR 생성 → squash merge → done 코멘트 → 라벨 제거 | PR merged, 이슈 closed |
| **6. 다음 이슈** | Claude | 남은 이슈 확인 → 있으면 1단계부터 반복 | 모든 이슈 완료 |

### Codex 리뷰 요청 템플릿

**방향성 리뷰 (1단계):**
```
이슈 #{number} 작업 방향을 리뷰해줘.
[이슈 요약 + 구현 방향 + 질문]
문제가 없으면 "방향 LGTM"으로 응답해.
```
...
변경 파일: {파일 목록}
변경 목적: {무엇을 왜 변경했는지}
확인 사항: 로직 오류, 회귀, 타입 안전성, 성능, 보안
문제가 있으면 구체적으로 지적하고 수정 방향을 제안해.
문제가 없으면 "LGTM"으로 응답해.
```

### 자동 실행 조건

아래 상황에서는 사용자 확인 없이 워크플로우를 자동 진행한다:
- 사용자가 "이슈 처리해", "작업 시작해" 등 작업 지시를 한 경우
- 이전 이슈 완료 후 남은 이슈가 있고, 사용자가 "남은 이슈도 처리해"라고 한 경우
- 사용자가 여러 이슈를 순차 처리하라고 지시한 경우

### 중단 조건

아래 상황에서는 사용자에게 확인을 구한다:
- Codex 리뷰에서 아키텍처 변경이 필요한 High 이슈가 나온 경우
- 이슈 범위가 불명확하여 요구사항 확인이 필요한 경우
- merge conflict가 자동 해결이 어려운 경우
