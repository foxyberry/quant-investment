...
### Phase 2: 조건 감지
- [ ] **#20** Condition Trigger Detection
  - 가격 목표 도달 감지
  - 쿨다운 기간 설정
  - 이벤트 로깅
- [ ] **#21** Trading Condition IoC Pattern
  - `TradingCondition` 프로토콜 정의
  - 기본 조건 구현 (손절, 익절, 트레일링)
  - 조건 체이닝

### Phase 3: 주문 실행
- [ ] **#22** Buy/Sell Quantity Configuration
  - 고정 수량, 퍼센트, 금액 기반 계산
  - 최소 주문 수량 처리
- [ ] **#23** Paper Trading (Dry-run)
  - 모의 주문 실행
  - 가상 포트폴리오 업데이트
  - 거래 로그 생성

### Phase 4: 확장 (선택)
- [ ] **#24** Live Order Execution (API 키 필요)
  - 한국투자증권/키움 API 연동
- [ ] **#25** Risk Management Rules Engine
  - 최대 포지션 한도
  - 일일 손실 한도
- [ ] **#26** Notification System (API 키 필요)
  - 텔레그램 봇 연동
